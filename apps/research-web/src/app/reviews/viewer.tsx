...
function sortReviews(reviews: Review[], sort: SortKey): Review[] {
  ...
  const dateValue = (r: Review) => (r.date ? new Date(r.date).getTime() : -Infinity);
  if (sort === "newest") {
  ...
  if (sort === "discussed") {
    return sorted.sort((a, b) => b.behavioralCodes.length - a.behavioralCodes.length);
  }
  if (sort === "negative") {
    return sorted.sort((a, b) => Number(b.sentiment === "negative") - Number(a.sentiment === "negative"));
  }
  if (sort === "positive") {
    return sorted.sort((a, b) => Number(b.sentiment === "positive") - Number(a.sentiment === "positive"));
  }
  const codeCount = new Map<string, number>();
  for (const r of reviews) for (const code of r.behavioralCodes) codeCount.set(code, (codeCount.get(code) ?? 0) + 1);
  return sorted.sort((a, b) => {
    const aTop = a.behavioralCodes.reduce((max, code) => Math.max(max, codeCount.get(code) ?? 0), 0);
    const bTop = b.behavioralCodes.reduce((max, code) => Math.max(max, codeCount.get(code) ?? 0), 0);
    return bTop - aTop;
  });
}
